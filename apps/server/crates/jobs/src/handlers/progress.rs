...
//! write's lateness settle, BUG-312), `progress:learner` (a grader's change to
//! one learner's work, BUG-313/314). Retries with backoff until it lands; the
//! projection is idempotent.

use ab_core::Result;
use ab_domain::progress::ProgressProjector;
use ab_domain::progress::projector::{
    COURSE_CHANGE_JOB, LATENESS_JOB, LEARNER_JOB, STAFF_CHANGE_JOB,
};
...
impl ProgressJob {
    ...
    #[must_use]
    pub const fn learner(pool: PgPool) -> Self {
        Self {
            pool,
            kind: LEARNER_JOB,
        }
