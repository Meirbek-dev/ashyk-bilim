...
/// Legacy `_draft_course_answer` (the ru strings verbatim), in the language
/// the client asked for (`language`, unless `auto`) or the caller's locale.
#[must_use]
pub fn draft_course_answer(language: &str, locale: Option<&str>) -> CourseQaAnswer {
    let hint = Some(language)
        .filter(|l| !l.eq_ignore_ascii_case("auto"))
        .or(locale);
    let (answer, context, note, suggestions) = match resolve_locale(hint) {
        Locale::Ru => (
            "Вопросы и ответы по курсу с использованием ИИ еще не включены. Вопрос был записан, но ответ от провайдера не был сгенерирован.",
            "Контекст курса",
        ...
        Locale::Kk => (
            "Курс бойынша ЖИ сұрақ-жауап әлі қосылмаған. Сұрақ жазылды, бірақ провайдерден жауап жасалмады.",
            "Курс контексті",
            "Сұраққа жауаптың жобасы модельге қолжетімсіз жасалды.",
            [
                "Бұл сұраққа оқытушыдан жауап беруін сұрау",
                "Ағымдағы дәріс конспектілерін қарау",
            ],
        ),
        Locale::Root => (
            "AI course Q&A is not enabled yet. The question was recorded, but no answer was generated by the provider.",
            "Course context",
            "A draft answer was produced without access to the model.",
            [
                "Ask the teacher to answer this question",
                "Review the current lecture notes",
            ],
        ),
    };
    CourseQaAnswer {
        answer_markdown: answer.into(),
        citations: vec![draft_citation("qa-draft", context, "course", note)],
        confidence: Level::Low,
        out_of_scope: false,
        follow_up_suggestions: suggestions.map(Into::into).to_vec(),
    }
}
...
mod tests {
    ...
    #[test]
    fn draft_answer_is_the_legacy_text_in_the_callers_language() {
        let draft = draft_course_answer("ru", None);
        ...
        // `auto` follows the caller's locale; an explicit language wins.
        assert!(
            draft_course_answer("auto", Some("kk-KZ"))
                .answer_markdown
                .starts_with("Курс бойынша")
        );
        assert!(
            draft_course_answer("en", Some("kk-KZ"))
                .answer_markdown
                .starts_with("AI course")
        );
        assert!(
            draft_course_answer("auto", None)
                .answer_markdown
                .starts_with("AI course")
        );
    }
