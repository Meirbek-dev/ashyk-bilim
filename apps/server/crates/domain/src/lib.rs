...
//! `identity` · `org` · `catalog` · `progress` · `assessment` · `grading` ·
//! `files` · `code` · `community` · `certs` · `gamification` · `analytics` ·
//! `ai` · `search` · `work` · `events`
//!
//! Rules (ARCHITECTURE §7):
//! - Every service method takes an `Actor` and performs its own permission
//!   checks — enforcement lives here, never only in handlers.
