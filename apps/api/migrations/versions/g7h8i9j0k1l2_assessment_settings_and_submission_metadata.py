...
def upgrade() -> None:
    ...
    if "exam" in existing_tables:
        op.execute("""
            ...
            WHERE exam.activity_id = activity.id
              AND activity.activity_type::text = 'TYPE_EXAM'
        """)
    ...
    op.execute("""
        ...
        WHERE activity.activity_type::text = 'TYPE_ASSIGNMENT'
    """)
    ...
    if "block" in existing_tables:
        op.execute("""
            ...
            WHERE block.activity_id = activity.id
              AND block.block_type::text = 'BLOCK_QUIZ'
        """)

    if "quiz_attempt" in existing_tables:
        op.execute("""
