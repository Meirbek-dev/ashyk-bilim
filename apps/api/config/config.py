...
class AIConfig(PlatformSectionSettings):
    ...
    openai_model: str = Field(
        default="gpt-5.6-luna", validation_alias="PLATFORM_OPENAI_MODEL"
    )
    ...
    max_tokens_per_request: int = Field(
        default=32_000, validation_alias="PLATFORM_AI_MAX_TOKENS_PER_REQUEST"
    )
    max_output_tokens: int = Field(
        default=16_000, validation_alias="PLATFORM_AI_MAX_OUTPUT_TOKENS"
    )
    monthly_token_budget: int = Field(
        default=1_000_000, validation_alias="PLATFORM_AI_MONTHLY_TOKEN_BUDGET"
    )
    ai_enabled: bool = Field(default=True, validation_alias="PLATFORM_AI_ENABLED")
    course_analysis_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_COURSE_ANALYSIS_ENABLED"
    )
    submission_analysis_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_SUBMISSION_ANALYSIS_ENABLED"
    )
    remediation_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_REMEDIATION_ENABLED"
    )
    course_qa_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_COURSE_QA_ENABLED"
    )
    study_companion_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_STUDY_COMPANION_ENABLED"
    )
    lecture_authoring_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_LECTURE_AUTHORING_ENABLED"
    )
    ai_draft_mode_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_DRAFT_MODE_ENABLED"
    )
    semantic_memory_enabled: bool = Field(
        default=True, validation_alias="PLATFORM_AI_SEMANTIC_MEMORY_ENABLED"
    )
