...
async def test_analytics_does_not_block_event_loop():
    ...
    with patch(
        "src.routers.analytics.get_teacher_overview", side_effect=mock_heavy_computation
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                return_value=MagicMock(course_ids=[1]),
            ):
                async with AsyncClient(
                    ...
                    assert health_duration < 0.2, (
                        f"Event loop was blocked for {health_duration}s"
                    )
...
async def test_all_analytics_endpoints_are_async():
    ...
    for route in router.routes:
        assert inspect.iscoroutinefunction(route.endpoint), (
            f"Endpoint {route.path} is not async"
        )
