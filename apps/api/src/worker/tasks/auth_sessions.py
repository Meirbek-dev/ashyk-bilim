...
async def audit_session_created_task(session_data: JsonObject) -> None:
    ...
    try:
        await asyncio.to_thread(audit_create_sync, session_data)
    except Exception:
        logger.exception(
...
async def audit_session_revoked_task(session_id: str) -> None:
    from src.services.auth.sessions import audit_revoke_sync

    try:
        await asyncio.to_thread(audit_revoke_sync, session_id)
    except Exception:
