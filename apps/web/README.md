...
- If the build fails with package manager errors, ensure `bun` is installed during the Docker build (the Dockerfile uses `oven/bun:1-alpine` base image). If building locally you can run `bun install` in `apps/web` to verify your lockfile.
