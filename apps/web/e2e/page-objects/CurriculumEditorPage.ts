...
export class CurriculumEditorPage {
  ...
  public async addActivityToChapter(chapterName: string, activityType: string): Promise<void> {
    ...
    const expandButton = chapterRow.getByRole('button', {
      name: /expand|open|show/i,
    })
    ...
    await this.page.waitForResponse(resp => resp.url().includes('/activities') && resp.request().method() === 'POST', {
      timeout: 10_000,
    })
  }
