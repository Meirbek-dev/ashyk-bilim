...
vi.mock('@/components/ui/AppLink', () => ({ default: ({ children }: { children?: React.ReactNode }) => <a>{children}</a> }))
...
vi.mock('@/hooks/useNavigationPermissions', () => ({
  useNavigationPermissions: () => ({ canSeeCourses: false, canSeeAnalytics: false, canSeeUsers: false, canSeeAdmin: false }),
}))
vi.mock('@/hooks/useSession', () => ({ useSession: () => ({ user: { id: 'u1', username: 'aigerim', display_name: 'Aigerim' } }) }))
