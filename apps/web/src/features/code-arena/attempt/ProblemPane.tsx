  ...
  return (
    <div className="bg-background flex h-full min-h-0 flex-col">
      ...
      >
        ...
        <TabsContent value="description" className="min-h-0 flex-1 overflow-hidden">
          <ScrollArea className="h-full">
            <div className="space-y-5 p-5">
              ...
              <MarkdownContent content={promptFallback} mode="codeProblem" />
              ...
              {visibleTests.length > 0 ? (
                <section className="space-y-3">
                  ...
                  {visibleTests.map((test, index) => (
                    <div key={`${test.id}-${index}`} className="bg-card rounded-md border">
                ...
                <section className="space-y-2">
                  ...
                  <ul className="space-y-1.5">
                    {problem.constraints.map((constraint, index) => (
                      <li key={`${constraint}-${index}`} className="flex items-start gap-2 text-sm">
        ...
        <TabsContent value="hints" className="min-h-0 flex-1 overflow-hidden">
          <ScrollArea className="h-full">
...
function SpecBlock({ title, content }: { title: string; content: string }) {
  return (
    <section className="space-y-2">
      ...
      <MarkdownContent content={content} mode="codeSpec" />
    </section>
  )
}
...
function CodeBlock({ label, children }: { label: string; children: string }) {
  return (
    <div>
      ...
      <MarkdownCodeBlock code={children} language="text" compact />
    </div>
  )
}
