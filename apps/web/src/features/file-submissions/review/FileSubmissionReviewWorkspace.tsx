...
export default function FileSubmissionReviewWorkspace({
  ...
  const selected =
    filteredItems.find((attempt) => attempt.attempt_uuid === selectedUuid) ??
    filteredItems[0] ??
    null;
  ...
  function selectAttempt(attempt: FileSubmissionAttempt) {
    ...
    setFeedback(
      typeof attempt.feedback?.feedback === "string"
        ...
        <div className="border-border sticky top-0 z-10 space-y-3 border-b bg-inherit p-4 backdrop-blur">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              render={
                <a
                  href={fileSubmissionExportUrl(config.file_submission_uuid)}
                />
              }
            >
            ...
            filteredItems.map((attempt) => (
              <button
                ...
                className={`hover:bg-muted/60 block w-full p-4 text-left transition-colors ${
                  selected?.attempt_uuid === attempt.attempt_uuid
                    ? "bg-muted"
                    : ""
                }`}
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-muted-foreground text-xs">
                      Attempt {attempt.attempt_number} · {attempt.files.length}{" "}
                      file
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Button
                      variant="outline"
                      onClick={() =>
                        gradeMutation.mutate({ status: "RETURNED" })
                      }
                      disabled={gradeMutation.isPending}
                    >
                    ...
                    <Button
                      variant="outline"
                      onClick={() =>
                        gradeMutation.mutate({ status: "PUBLISHED" })
                      }
                      disabled={gradeMutation.isPending}
                    >
                      Publish result
