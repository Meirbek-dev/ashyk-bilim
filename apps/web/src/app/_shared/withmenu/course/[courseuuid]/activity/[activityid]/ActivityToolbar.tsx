...
export default function ActivityToolbar({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      {showNavigation ? (
        <NextActivityButton course={course} currentActivityId={activity.id} />
      ) : null}
    </div>
  );
}
  ...
  async function markComplete() {
    try {
      ...
      const willCompleteAll =
        completedActivityIds.size >= totalActivityCount - 1;
      ...
      if (useGamificationStore.getState().profile) {
        ...
        refetchGamification().catch((error: unknown) =>
      ...
      if (willCompleteAll) {
        router.push(
          `${getAbsoluteUrl("")}/course/${cleanCourseUuid}/activity/end`,
        );
      }
    } catch (error) {
...
function NextActivityButton({
  ...
  const currentIndex =
    activityIndex.indexByActivityId.get(currentActivityId) ?? -1;
  const nextActivity =
...
function PreviousActivityButton({
  ...
  const previousActivity =
    currentIndex > 0 ? activityIndex.allActivities[currentIndex - 1] : null;
