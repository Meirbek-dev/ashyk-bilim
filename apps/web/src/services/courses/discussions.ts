// Client-safe (no 'use server'): the course page keeps discussions in a
// TanStack query that polls while the thread is open (UX-062); problem+json
// codes reach the client through apiJson (BUG-035).
  ...
  return {
    ...
    ...(value.author
      ? {
          user: {
            id: value.author.id,
            user_uuid: value.author.id,
            username: value.author.username,
            first_name: value.author.display_name,
            last_name: '',
            email: '',
            avatar_image: value.author.avatar_key ?? '',
          },
        }
      : {}),
...
export async function getCourseDiscussions(courseId: string, includeReplies = true, limit = 50): Promise<Discussion[]> {
  const items = await collectPages(cursor =>
    apiJson(
      `courses/${courseId}/discussions?include_replies=${includeReplies}&limit=${limit}${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ''}`,
      {},
      data => DiscussionPage.parse(data),
    ),
  )
...
export async function createDiscussion(courseId: string, discussion: DiscussionCreate): Promise<Discussion> {
  const data = await apiJson(
    `courses/${courseId}/discussions`,
    {
      ...
      body: JSON.stringify({ content: discussion.content, parent_id: discussion.parent_discussion_id ?? null }),
    },
    data => WireDiscussion.parse(data),
  )
...
export async function updateDiscussion(
  ...
  const data = await apiJson(
    `discussions/${id}`,
    { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(discussion) },
    data => WireDiscussion.parse(data),
  )
  return normalize(data)
}
