...
export function HeaderProfileBox() {
  ...
  const highestRole = SYSTEM_ROLE_PRIORITY.find(slug => userRoles.includes(slug))
  const userRoleInfo: RoleInfo | null =
    userRoles.length > 0 ? (roleConfigs[highestRole ?? RoleSlugs.USER] ?? roleConfigs[RoleSlugs.USER] ?? null) : null
