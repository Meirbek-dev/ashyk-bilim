    ...
    >
      <div className="border-border bg-popover flex h-auto w-max items-center gap-2 rounded-lg border px-3 py-1.5 shadow-md">
        ...
        <div className="flex items-center gap-0.5">
          <AIActionButton editor={props.editor} label="Explain" />
          <AIActionButton editor={props.editor} label="Summarize" />
          <AIActionButton editor={props.editor} label="Translate" />
          <AIActionButton editor={props.editor} label="Examples" />
        </div>
      </div>
    </BubbleMenu>
  )
...
const AIActionButton = (props: { editor: Editor; label: ActionLabel }) => {
  ...
  return (
    <ToolTip sideOffset={10} slateBlack content={getTooltipLabel(props.label)}>
      <Button
        variant="ghost"
        size="sm"
